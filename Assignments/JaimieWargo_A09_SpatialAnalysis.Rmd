...
mapview(ne_counties_sf, layer.name='NE Counties')
```
...
> ANSWER: The EPSG code is 4269. This uses angular units, which we know
> because the HTML specifies units in degrees, or projected coordinate
> system. This CRS is associated with the North American Datum 1983
> (NAD83).
...
> ANSWER: The x coordinate is longitude, so dec_long_va is the column
> for that. The y coordinate is therefore latitude, which is dec_lat_va.
...
> ANSWER: The new field that appeared in the geometry, which has the
> coordinates in it as specified. The fields that disappeared were the
> lat and long variables since those were combined into the geometry
> field.
...
ggplot() + 
  geom_sf(data=ne_counties_sf, aes(fill=STATEFP), fill='lightskyblue') +
  geom_sf(data=gage_sf, color='darkblue')+
  labs(title = "NWIS Gage Locations in Nebraska",
       subtitle = "Jaimie Wargo")
```
...
NWIS_flow_data <- read.csv(here('Data', 'Raw', 'NWIS_SiteFlowData_NE_RAW.csv'),
                           colClasses = c('site_no'='factor'),
                           stringsAsFactors = TRUE)
...
NWIS_NE_gage_join <- gage_sf %>% 
  left_join(NWIS_flow_data, by = 'site_no') %>% 
  drop_na()


#17. Show the column names of the joined dataset
colnames(NWIS_NE_gage_join)

#18. Show the dimensions of this joined dataset
dim(NWIS_NE_gage_join)
...
mapview(ne_counties_sf, col.regions='lightpink', layer.name='Counties')+
  mapview(NWIS_NE_gage_join, cex='gage_ht', col.regions='purple3', 
...
counties_utm14 <- st_transform(ne_counties_sf, crs = 32614)
gage_utm14 <- st_transform(gage_sf, crs = 32614)

#21 Plot the data
mapview(counties_utm14, col.regions='lightsalmon', layer.name='Counties')+
  mapview(gage_utm14, col.regions='tomato2', layer.name='Gage Locations')
...
Dawson_county <- counties_utm14 %>% 
  filter(NAME=='Dawson')

#23 Spatially select gages within the selected county
gage_intersect <- gage_utm14[Dawson_county,]

#24 Plot
mapview(counties_utm14, layer.name='Counties', col.regions='palegreen3')+
  mapview(Dawson_county, layer.name='Dawson County', 
          col.regions='royalblue4')+
  mapview(gage_intersect, layer.name='Dawson Gages', col.regions='darkorange1')
