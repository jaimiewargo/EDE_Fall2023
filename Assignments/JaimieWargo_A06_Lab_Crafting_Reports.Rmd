...
```{r project.setup, message=F, error=F, warning=F}
#Import libraries
...
```{r epa.month.to.factor, echo=F, warning=F, message=F, error=F}
epa_data$Month <- factor(epa_data$Month, 
  levels=c(1:12),labels=month.abb)
```

```{r O3 2018, echo=F, fig.height = 4, fig.cap="Monthly Ozone, 2018"}
data_18 <- epa_data%>% 
  drop_na(Ozone) %>%
  filter(Year==2018)

o3_plot <- ggplot(data_18, aes(x=Month, y=Ozone, group=Month))+
  geom_boxplot()+
  labs(
    x="Month",
...
```{r O3 2019, echo=F, fig.height=4, fig.cap="Monthly Ozone, 2019"}
data_19 <- epa_data%>% 
  drop_na(Ozone) %>%
  filter(Year==2019)

o3_plot <- ggplot(data_19, aes(x=Month, y=Ozone, group=Month))+
  geom_boxplot()+
  labs(
    x="Month",
    y=expression(paste("Concentration (", mu, "g/", m^3, ")")),
...
<https://github.com/jaimiewargo/EDE_Fall2023>
