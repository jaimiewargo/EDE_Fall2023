...
> Answer: This vector is not numeric because there are values within it that contain character values, such as \~, /, and NR. Vectors can only have one data type, so it defaults to the most universal, which is character-- more specifically, the class is factor.
