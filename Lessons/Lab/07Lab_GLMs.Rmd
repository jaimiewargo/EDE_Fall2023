...
ozone_by_year <- ggplot(data=EPAair, aes(x=Ozone, color=Year, fill=Year))+
  geom_density(alpha=0.4)+
  scale_color_manual(values = c("lightsteelblue", "plum"))+
...
ozone_by_year_box <- ggplot(data=EPAair, aes(y=Ozone, x=Year))+
  geom_boxplot()+
...
ozone_by_year_violin <- ggplot(data=EPAair, aes(y=Ozone, x=Year))+
...
PM25_Ozone <- lm(data=EPAair, PM2.5 ~ Ozone)
summary(PM25_Ozone)

#R-squared is 0.1316, p value is close to 0
...
PM25_O3_scatter <- ggplot(EPAair, aes(x=Ozone, y=PM2.5))+
