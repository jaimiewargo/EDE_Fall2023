...
-   **Data Frames** 2 dimensional structure that is more general than a matrix. Columns can have different modes (e.g., numeric and factor). When we import csv files into the R workspace, they will enter as data frames.
    ...
    -   *Note: specify stringAsFactors = True when importing files*
...
> Answer: "num" for numerical data, "chr" for character, "logi" for logical, "int" for integer

Question: In the R chunk below, write "dataframe1\$". Press `tab` after you type the dollar sign. What happens?

> Answer: It fills in the ID column since it is the first column in the data frame.
...
```{r DataExercise-1a}
durham_highs <- c(79, 83, 83, 84, 78, 71, 79, 82, 85, 84)
durham_lows <- c(55, 56, 58, 60, 61, 63, 63, 64, 64, 63)
print('Highs')
durham_highs
print('Lows')
durham_lows
...
```{r DataExercise-1b}
F_to_C <- function(temperature){
  temp_C <- (5/9)*(temperature-32)
  return(temp_C)
}

durham_high_C <- round(F_to_C(durham_highs),2)
print(durham_high_C)

durham_low_C <- round(F_to_C(durham_lows),2)
print(durham_low_C)
```
...
```{r DataExercise-1c}
durham_temps <- as.data.frame(cbind(durham_highs, durham_high_C,
                                durham_lows, durham_low_C))
...
#specify format like as.Date, specify cutoff
date_obj_past

#Alternative 2
date_obj_past2 <- parse_date_time2(str_past,"ymd",cutoff_2000=23L)
#specify format in lubridate style, specify cutoff
date_obj_past2
