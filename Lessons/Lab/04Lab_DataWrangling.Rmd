...
NTL.phy.data.1999 <- filter(NTL.phys.data, year4 == 1999)

# Exercise 2: 
# filter NTL.phys.data for Tuesday Lake from 1990 through 1999.

NTL.phy.data.Tuesday <- filter(NTL.phys.data, year4 %in% c(1990:1999)
...
NTL.phy.data.JulyTuesday90s <- 
  NTL.phys.data %>% 
  mutate(samplemonth = month(sampledate)) %>% 
  filter(year4 %in% c(1990:1999)& lakename == 'Tuesday Lake' & samplemonth==7) 
