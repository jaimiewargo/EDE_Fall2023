...
sum(is.na(BR_complete$Rainfall))
sum(is.na(BR_complete$Temperature_C))
#Both are 0, no NAs
...
Rainfall_Time <- ggplot(data=BR_complete, aes(x=Date, y=Rainfall))+
  geom_line()+
  geom_smooth(method="lm", se=F)

Rainfall_Time

Temp_Time <- ggplot(data=BR_complete, aes(x=Date, y=Temperature_C))+
  geom_line()+
  geom_smooth(method="lm", se=F)
...
Temp_ts.Decomposed <- stl(Temp_ts, s.window = "periodic")
plot(Temp_ts.Decomposed)
...
Rainfall_ts.Components <- as.data.frame(Rainfall_ts.Decomposed$time.series[,1:3])
Rainfall_ts.Components <- mutate(Rainfall_ts.Components,
        Observed = BR_complete$Rainfall,     
        Date = BR_complete$Date)

Temp_ts.Components <- as.data.frame(Temp_ts.Decomposed$time.series[,1:3])
Temp_ts.Components <- mutate(Temp_ts.Components,
        Observed = BR_complete$Temperature_C,     
        Date = BR_complete$Date)


# Exercise 10: Visualize how the trend maps onto the data for both series
ggplot(Rainfall_ts.Components) +
  geom_line(aes(y = Observed, x = Date),  size = 0.25) +
  geom_line(aes(y = trend, x = Date), color = "blue") +
  geom_hline(yintercept = 0, lty = 2) +
  ylab(expression("Rainfall"))

ggplot(Temp_ts.Components) +
  geom_line(aes(y = Observed, x = Date),  size = 0.25) +
  geom_line(aes(y = trend, x = Date), color = "darkorange") +
  geom_hline(yintercept = 0, lty = 2) +
